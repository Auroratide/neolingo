...
<fieldset>
	...
	{#each words as word (word.id)}
		<li>
			<input id="{id}-{word.id}" type="radio" name="{id}" value={word.id} class="checkmark" />
...
<style>
	...
	ul:has(input:checked) input:not(:checked),
	ul:has(input:checked) input:not(:checked) + label {
		opacity: 0.667;
	}
